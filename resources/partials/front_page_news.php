...
<?php
    if ($_SERVER['SERVER_NAME'] == "localhost") {
      ...
      $host = "eastcoastmilitia.db.9202306.hostedresource.com";
      $username = "eastcoastmilitia";
      $password = "Kidrock24!";
      $db_name = "eastcoastmilitia";
    }
    ...
    $result = mysqli_query($con, "SELECT * FROM news_stories ORDER BY date_added DESC LIMIT 7");
    
    if ($first_row = mysqli_fetch_array($result)) { 
      if (strpos($first_row['media'], "www.facebook.com/video")) {
        $top_story_img = "<iframe src='".$first_row['images']."' framborder='0'></iframe>";
      }
      else {
        $top_story_img = "<img src='/resources/images/news_images/".$first_row['media']."' />";
      }
    ...
    while ($row = mysqli_fetch_array($result)) {
      if (strpos($row['media'], "www.facebook.com/video")) {
        $media = "<iframe src='".$row['media']."' frameborder='0'></iframe>";
      } else {
        $media = "<img src='/resources/images/news_images/".$row['media']."' />";
      }
      $caption = $row['title'];
  ...
  <div id="top-story">
    ...
    <div id="top-story-img">
      <img src="/resources/images/militia_store.png">
    </div>
  ...
  <?php include $root_path.'/resources/partials/front_page_extras.html'; ?>
</div>
